...
class VariableSequenceClassification:
    ...
    def __init__(self, weibo, data, data_len, target, num_hidden=100, num_layers=2, weibo_len=weibo_len,
        ...
        self.embedded_chars = tf.nn.embedding_lookup(self.W, self.weibo)
        self.embedded_chars_expanded = tf.expand_dims(self.embedded_chars, -1)
        self.weibo_shape = tf.shape(self.embedded_chars_expanded)
...
class DataTensor:
    ...
    def feed(self):
        return {weibo: self.weibo, data: self.data, data_len: self.len, target: self.tensor}
...
if __name__ == '__main__':
    ...
    model = VariableSequenceClassification(weibo, data, data_len, target)

    sess = tf.Session()

    timestamp = str(int(time.time()))
    out_dir = os.path.abspath(os.path.join(os.path.curdir, "7_3_CDE_OM_runs", timestamp))
    print("Writing to {}\n".format(out_dir))
